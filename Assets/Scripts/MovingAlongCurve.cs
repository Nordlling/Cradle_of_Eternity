...
public class MovingAlongCurve : MonoBehaviour
{
    ...
    void Update()
    {
        if (_started)
        {
            ...
            transform.position = GetBexierPoint(_from, _through, _to, progress);

            if (progress >= 1 && _removeWhenFinished) Destroy(this);
        }
    }
    ...
    private Vector3 GetBexierPoint(Vector3 a, Vector3 b, Vector3 c, float t)
    {
